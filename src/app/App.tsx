...
const DEFAULT_AVATAR = "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=160&h=160&fit=crop&auto=format";
...
function PostCard({ post, onToggleLike, onToggleSave, onOpenComments }: { post: UIPost; onToggleLike: (id: string) => void; onToggleSave: (id: string) => void; onOpenComments: (p: UIPost) => void }) {
...
function ExploreTab({ posts, onOpenComments }: { posts: UIPost[]; onOpenComments: (p: UIPost) => void }) {
...
function ProfileTab({ profile, posts, onLogout }: { profile: Profile; posts: UIPost[]; onLogout: () => void }) {
  ...
  return (
    <div>
      <div className="relative h-28 rounded-2xl overflow-hidden mb-0"><img src="https://images.unsplash.com/photo-1477959858617-67f85cf4f1df?w=600&h=200&fit=crop&auto=format" alt="" className="w-full h-full object-cover" /><div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent, rgba(15,15,15,0.8))" }} /></div>
      <div className="flex items-end gap-4 -mt-10 mb-4 px-1">
        <div className="relative shrink-0">
          <div className="w-20 h-20 rounded-full overflow-hidden border-4" style={{ borderColor: "#f97316" }}>
          ...
          <button
            ...
            className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full flex items-center justify-center border-2 border-background shadow-lg"
            style={{ backgroundColor: "#f97316" }}
            aria-label="Trocar foto de perfil"
          >
            <Camera size={13} style={{ color: "#0f0f0f" }} />
          </button>
          <input ref={avatarInputRef} type="file" accept="image/*" className="hidden" onChange={handleFilePicked} />
        </div>
        <div className="flex-1 pb-1">
          <h2 className="text-base font-black text-foreground leading-none">{profile.full_name}</h2>
          <p className="text-xs text-muted-foreground mt-0.5">@{profile.username}{profile.district ? ` · ${profile.district}` : ""}</p>
        </div>
        <button onClick={() => setShowSettings(true)} className="p-2 rounded-xl bg-muted text-muted-foreground hover:text-foreground transition-colors pb-2" aria-label="Configurações">
          <Settings size={16} />
        </button>
      </div>
      {avatarError && <p className="text-xs font-semibold text-destructive px-1 mb-3">{avatarError}</p>}
      <div className="grid grid-cols-4 gap-2 mb-5">
        {[{ label: "Reportes", value: myPosts.length, icon: AlertTriangle, color: "#f97316" }, { label: "Resolvidos", value: resolved, icon: CheckCircle, color: "#22c55e" }, { label: "Curtidas", value: totalLikes, icon: Heart, color: "#ef4444" }, { label: "Pontos", value: profile.points, icon: Award, color: "#a78bfa" }].map(({ label, value, icon: Icon, color }) => (
      ...
      {display.length === 0 ? <div className="text-center py-10 text-muted-foreground"><Bookmark size={32} className="mx-auto mb-2 opacity-30" /><p className="text-sm font-semibold">{pTab === "saved" ? "Nenhuma ocorrência salva" : "Você ainda não reportou nada"}</p></div>
      : view === "grid" ? (
        <div className="grid grid-cols-3 gap-1.5">
      ...
      <AnimatePresence>
        {cropFile && <AvatarCropModal file={cropFile} saving={uploadingAvatar} onClose={() => setCropFile(null)} onConfirm={handleCropConfirm} />}
        {showSettings && <SettingsModal profile={profile} onClose={() => setShowSettings(false)} onLogout={onLogout} />}
      </AnimatePresence>
    </div>
  );
}
        ...
        <main className="flex-1 overflow-y-auto scrollbar-hide">
          <div className="max-w-lg mx-auto py-6 px-4">
            {activeTab === "feed" && <>
              ...
              <div className="space-y-5">{filtered.map((p) => <PostCard key={p.id} post={p} onToggleLike={toggleLike} onToggleSave={toggleSave} onOpenComments={openComments} />)}</div>
            </>}
            {activeTab === "buscar" && <ExploreTab posts={posts} onOpenComments={openComments} />}
            {activeTab === "notif"  && <NotifTab notifs={notifs} onMarkAll={onMarkAllRead} />}
            {activeTab === "perfil" && <ProfileTab profile={profile} posts={posts} onLogout={onLogout} />}
          </div>
        </main>
        ...
        <main className="flex-1 overflow-y-auto scrollbar-hide">
          <div className="px-3 py-4">
            {activeTab === "feed" && <>
              ...
              {filtered.length === 0 && <div className="text-center py-16 text-muted-foreground"><AlertTriangle size={32} className="mx-auto mb-2 opacity-30" /><p className="text-sm font-semibold">Nenhuma ocorrência por aqui ainda</p></div>}
              <div className="space-y-4">{filtered.map((p) => <PostCard key={p.id} post={p} onToggleLike={toggleLike} onToggleSave={toggleSave} onOpenComments={openComments} />)}</div>
            </>}
            {activeTab === "buscar" && <ExploreTab posts={posts} onOpenComments={openComments} />}
            {activeTab === "notif"  && <NotifTab notifs={notifs} onMarkAll={onMarkAllRead} />}
            {activeTab === "perfil" && <ProfileTab profile={profile} posts={posts} onLogout={onLogout} />}
